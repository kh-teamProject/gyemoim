...
const MyPage = () => {
  ...
  useEffect(() => {
    axios.get('/mypage', {
      ...
      .then((res) => {
        ...
        if (res.data.monthlySalary !== undefined) {
  ...
  return (
    <section>
      ...
      <div>
        <h2 className={classes.memberText}>개인정보</h2>
        <div className={classes.field}>
          <ul>
            <li>
              <NavLink to={'/mypage/info'}
                       className={`${location.pathname.includes('info') ? classes.isActive : undefined}`}>개인정보
                수정</NavLink>
            </li>
            <li>
            ...
            <li>
              <NavLink to={'/mypage/info/delete/3'}
